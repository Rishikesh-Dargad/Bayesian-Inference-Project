                         ...
                         nsim, burnin, pivotal.z = NULL)
{
  ...
  GAMMA <- matrix(NA, nrow = nsim - burnin, ncol = G)
  ...
  for(i in 1:nsim){
    ...
    for(l in 1:G) 
    {
      ...
      if(type == "CN"){
        ...
        ui.l    <- ui[z == l]
        n.gamma <- sum(ui.l == gamma.l)
        m1 <- n.gamma
        m2 <- n.l - n.gamma
        nu.list[l] <- qbeta(
          runif(1,
                pbeta(0.01, nu01 + m1, nu02 + m2),
                pbeta(0.5,  nu01 + m1, nu02 + m2)
          ),
          shape1 = nu01 + m1,
          shape2 = nu02 + m2
        )
        
        ## 4) update the variance‐inflation factor gamma_l if there is at least one “good” obs
        if(n.gamma > 0){
          Y.lb    <- Y.l[ui.l == gamma.l]
          X.lb    <- X.l[ui.l == gamma.l, , drop = FALSE]
          meany.l <- as.numeric(
            t(Y.lb - X.lb %*% Bn.l) %*% (Y.lb - X.lb %*% Bn.l)
          ) / (2 * sig2.l)
          
          gamma.list[l] <- qgamma(
            runif(1,
                  pgamma(0.001, shape = 1 + 0.5 * n.gamma, rate = meany.l),
    ...
    if(G == 2){
      ...
      z[z == 0] = 2
      z = factor(z,levels = 1:G)
      tau = cbind( as.vector(result.upg$posterior$beta), rep(0, q1) )
